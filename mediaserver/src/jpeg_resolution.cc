...
static void get_jpeg_resolution(Ref<IOHandler> ioh, int *w, int *h)
{
    ...
    if (a != 0xff || ioh_fgetc(ioh) != M_SOI)
        throw Exception("get_jpeg_resolution: could not read jpeg specs");
    ...
    for(;;)
    {
        ...
        for (a = 0; a < 7; a++)
        {
            ...
            if (a >= 6)
                throw Exception("get_jpeg_resolution: too many padding bytes");
        }
        ...
        if (itemlen < 2)
            throw Exception("get_jpeg_resolution: invalid marker");
        ...
        if (got != itemlen-2)
            throw Exception("get_jpeg_resolution: Premature end of file?");
        ...
        switch(marker)
        {
            case M_EOI:   // in case it's a tables-only JPEG stream
                throw Exception("get_jpeg_resolution: No image in jpeg!");
            case M_SOF0: 
            case M_SOF1: 
            case M_SOF2: 
            case M_SOF3: 
    ...
    throw Exception("get_jpeg_resolution: resolution not found");
}
