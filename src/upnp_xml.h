...
public:
    ...
    void renderObject(const std::shared_ptr<CdsObject>& obj, std::size_t stringLimit, pugi::xml_node& parent, std::optional<std::reference_wrapper<Quirks>> quirks = std::nullopt) const;
    ...
    void addResources(const std::shared_ptr<CdsItem>& item, pugi::xml_node& parent, const Quirks& quirks) const;
...
protected:
    ...
    std::pair<bool, int> insertTempTranscodingResource(const std::shared_ptr<CdsItem>& item, const Quirks& quirks, std::deque<std::shared_ptr<CdsResource>>& orderedResources, bool skipURL) const;
