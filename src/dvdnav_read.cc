...
static const char* audio_format[7] = { "ac3", "?", "mpeg1", "mpeg2", "lpcm ", "sdds ", "dts" };
...
DVDNavReader::DVDNavReader(String path)
{
    ...
    if (dvdnav_set_PGC_positioning_flag(dvd, 1) != DVDNAV_STATUS_OK) {
        throw _Exception(_("Failed to set PGC positioning flag on DVD ") + path);
    }
...
int DVDNavReader::titleCount()
{
    ...
    if (dvdnav_get_number_of_titles(dvd, &t) != DVDNAV_STATUS_OK)
        throw _Exception(_("Failed to get title count for DVD ") + dvd_path + " : " + dvdnav_err_to_string(dvd));
...
void DVDNavReader::selectPGC(int title_idx, int chapter_idx)
{
    ...
    if (dvdnav_part_play(dvd, title_idx, chapter_idx) != DVDNAV_STATUS_OK) {
        throw _Exception(_("Failed to select PGC for DVD ") + dvd_path + " : " + dvdnav_err_to_string(dvd));
    }
...
size_t DVDNavReader::readSector(unsigned char* buffer, size_t length)
{
    ...
    while (!EOT) {
        ...
        if (result == DVDNAV_STATUS_ERR) {
            throw _Exception(_("Error getting next block for DVD ") + dvd_path + " : " + dvdnav_err_to_string(dvd));
        }
...
String DVDNavReader::audioLanguage(int stream_idx)
{
    ...
    if (dvdnav_get_audio_attr(dvd, stream_idx, &audio_attr) != DVDNAV_STATUS_OK)
        throw _Exception(_("Error error retrieving audio language from DVD ") + dvd_path + " : " + dvdnav_err_to_string(dvd));
