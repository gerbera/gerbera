...
protected:
    ...
    GrbFile file;
    /// @brief Folder for temporary file
    fs::path tmpDir;
    /// @brief full temporary file
    fs::path outFilePath;
    /// @brief Profile for transcoding
    std::shared_ptr<TranscodingProfile> profile;
    /// @brief Object in handler
    ...
    /// @brief open input file and streams
    int open_input_file();
    /// @brief create output file and streams
    /// @param filename full path to file, if empty output is sent to buffer only
    int open_output_file(const std::string& filename);
    /// @brief create filter per stream
    int init_filters(void);
    /// @brief send remaining data to output before closing handler
    ...
    /// @brief cleanup before terminating handler
    int closeOutput();
