...
TranscodeExternalHandler::TranscodeExternalHandler()
    : TranscodeHandler()
{
}
...
//info->content_type = ixmlCloneDOMString(mimeType.c_str());
    ...
    if (service == OS_SopCast) {
        ...
        sop_args = parseCommandLine(location + " " + String::from(p1) + " " + String::from(p2), nullptr, nullptr);
...
#warning check if we can use "accept url" with sopcast
    ...
    if (profile->getCommand().startsWith(_(_DIR_SEPARATOR))) {
        if (!check_path(profile->getCommand()))
            throw _Exception(_("Could not find transcoder: ") + profile->getCommand());
        ...
        if (!string_ok(check))
            throw _Exception(_("Could not find transcoder ") + profile->getCommand() + " in $PATH");
    }

    int err = 0;
    if (!is_executable(check, &err))
        throw _Exception(_("Transcoder ") + profile->getCommand() + " is not executable: " + strerror(err));
