...
private:
    ...
    public:
        Wd(fs::path path, int wd, int parentWd)
            : wdWatches(std::make_shared<std::list<std::shared_ptr<Watch>>>())
            , path(std::move(path))
            , parentWd(parentWd)
            , wd(wd)
        {
        ...
        std::shared_ptr<std::list<std::shared_ptr<Watch>>> getWdWatches() const { return wdWatches; }
        void addWatch(std::shared_ptr<Watch> w) { wdWatches->push_back(move(w)); }

    private:
        std::shared_ptr<std::list<std::shared_ptr<Watch>>> wdWatches;
        fs::path path;
        int parentWd;
        int wd;
    };
