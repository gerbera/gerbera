...
duk_ret_t jsAddCueObject(duk_context* ctx)
{
    ...
    try {
        ...
        ScriptProperty(ctx, -1).getObject([&]() {
            ...
            entry.discArtist = ScriptNamedProperty(ctx, "discArtist").getStringValue();
        ...
        if (parseTime(offset, entry.offset))
            resource->addAttribute(ResourceAttribute::OFFSET, offset * OFFSET_FACTOR + entry.frames); // Some specs say 75 frames per second
        if (cdsObj->isItem()) {
            auto item = std::static_pointer_cast<CdsItem>(cdsObj);
            if (item->getTrackNumber() == 0)
                item->setTrackNumber(entry.track);
