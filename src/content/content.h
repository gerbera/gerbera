...
public:
    ...
    virtual void parseMetafile(
        const std::shared_ptr<CdsObject>& obj,
        const fs::path& path) const
        = 0;
    /// @brief parse a file containing cuesheet
    virtual std::vector<int> parseCueSheet(
        const std::shared_ptr<CdsObject>& obj,
        const fs::path& path) const
        = 0;

    virtual bool isHiddenFile(
    ...
    virtual void rescanDirectory(
        const std::shared_ptr<AutoscanDirectory>& adir,
        int objectId,
        fs::path descPath = {},
        bool cancellable = true)
        = 0;
    virtual void handlePersistentAutoscanRecreate(
        const std::shared_ptr<AutoscanDirectory>& adir)
        = 0;
    virtual void triggerPlayHook(
