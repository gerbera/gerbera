...
js_copyObject(JSContext* cx, uintN argc, jsval* argv)
{
    ...
    try {
        ...
        JS_ARGV(cx, argv)
        [1] = OBJECT_TO_JSVAL(js_cds_clone_obj);
    ...
    try {
        ...
        if (self->whoami() == S_PLAYLIST) {
            ...
            if (!IS_CDS_ITEM_EXTERNAL_URL(otype) && !IS_CDS_ITEM_INTERNAL_URL(otype)) {
                ...
                if (string_ok(loc) && (IS_CDS_PURE_ITEM(otype) || IS_CDS_ACTIVE_ITEM(otype)))
        ...
        if ((self->whoami() == S_PLAYLIST) && (ConfigManager::getInstance()->getBoolOption(CFG_IMPORT_SCRIPTING_PLAYLIST_SCRIPT_LINK_OBJECTS))) {
