...
void YouTubeService::getPagingParams(Ref<Element> xml, Ref<YouTubeTask> task)
{
    ...
    if (!string_ok(temp))
        ...
        if (itmp <= 0) {
            throw _Exception(_("Tag <") + xml->getName() + _("> ") + _(CFG_OPTION_STARTINDEX) + _(" must be at least 1!"));
        }
    }
...
void YouTubeService::addTimeParams(Ref<Element> xml, Ref<YouTubeTask> task)
{
    ...
    if ((temp != GDATA_YT_VALUE_TIME_RANGE_ALL) && (temp != GDATA_YT_VALUE_TIME_RANGE_DAY) && (temp != GDATA_YT_VALUE_TIME_RANGE_WEEK) && (temp != GDATA_YT_VALUE_TIME_RANGE_MONTH))
        throw _Exception(_("Invalid time range parameter \"") + temp + _("\" in <") + xml->getName() + _("> tag"));
...
Ref<Object> YouTubeService::defineServiceTask(Ref<Element> xmlopt, Ref<Object> params)
{
    ...
    if (temp == CFG_REQUEST_STDFEED)
        ...
        throw _Exception(_("Unsupported tag while parsing YouTube options: ") + temp);
    ...
    case YT_request_stdfeed:
        ...
        if (temp2 != GDATA_REQUEST_STDFEED_WATCH_ON_MOBILE) {
            ...
            if (r < YT_region_none) {
                task->url_part = task->url_part + YT_regions[r].region_code + _("/");
                task->region = r;
            }
        }

        task->url_part = task->url_part + temp2;
        if ((temp2 != GDATA_REQUEST_STDFEED_MOST_RECENT) && (temp2 != GDATA_REQUEST_STDFEED_RECENTLY_FEATURED) && (temp2 != GDATA_REQUEST_STDFEED_WATCH_ON_MOBILE)) {
    ...
    case YT_request_user_favorites:
        ...
        task->url_part = _(GDATA_REQUEST_USERS) + temp + _(GDATA_REQUEST_FAVORITES);
    ...
    case YT_request_user_subscriptions:
        temp = getCheckAttr(xmlopt, _(CFG_OPTION_USER));
        task->url_part = _(GDATA_REQUEST_USERS) + temp + _(GDATA_REQUEST_SUBSCRIPTIONS);
        task->amount = AMOUNT_ALL;
        break;
    case YT_request_user_playlists:
        temp = getCheckAttr(xmlopt, _(CFG_OPTION_USER));
        task->url_part = _(GDATA_REQUEST_USERS) + temp + _(GDATA_REQUEST_PLAYLISTS);
        task->amount = AMOUNT_ALL;
        break;
    case YT_request_user_uploads:
        task->parameters->put(_(GDATA_YT_PARAM_FORMAT),
            _(GDATA_YT_VALUE_FORMAT_SWF));
        temp = getCheckAttr(xmlopt, _(CFG_OPTION_USER));
        task->url_part = _(GDATA_REQUEST_USERS) + temp + _(GDATA_REQUEST_UPLOADS);
...
bool YouTubeService::refreshServiceData(Ref<Layout> layout)
{
    ...
    if (task->amount_fetched < task->amount) {
        ...
        if ((task->amount - task->amount_fetched) >= _(GDATA_YT_VALUE_PER_PAGE_MAX).toInt()) {
            task->parameters->put(_(GDATA_YT_PARAM_MAX_RESULTS_PER_REQ),
                _(GDATA_YT_VALUE_PER_PAGE_MAX));
        } else if ((task->amount - task->amount_fetched) < _(GDATA_YT_VALUE_PER_PAGE_MAX).toInt()) {
            task->parameters->put(_(GDATA_YT_PARAM_MAX_RESULTS_PER_REQ),
    ...
    if ((task->request == YT_request_user_subscriptions) || (task->request == YT_request_user_playlists)) {
    ...
    if ((task->request == YT_subrequest_subscriptions) || (task->request == YT_subrequest_playlists))
        construct_url = false;
    ...
    do {
        ...
        if (old == nullptr) {
            ...
            if ((task->request == YT_subrequest_playlists) || (task->request == YT_subrequest_subscriptions)) {
