...
void FileRequestHandler::get_info(IN const char* filename, OUT UpnpFileInfo* info)
{
    ...
    if (slash_pos >= 0) {
        if (slash_pos < path.length() - 1) {
            ...
            header = _("Content-Disposition: attachment; filename=\"") + path.substring(slash_pos) + _("\"");
        }
    }
    ...
    if (((res_id > 0) && (res_id < item->getResourceCount())) || ((res_id > 0) && string_ok(rh))) {
        ...
        if (!is_srt && string_ok(tr_profile)) {
            ...
            if (tp == nullptr)
                throw _Exception(_("Transcoding of file ") + path + " but no profile matching the name " + tr_profile + " found");
