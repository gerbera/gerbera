        ...
        if (config->getBoolOption(CFG_SERVER_EXTEND_PROTOCOLINFO)) {
            ...
            if (contentType == CONTENT_TYPE_MP3)
                ...
                if (string_ok(resolution) && check_resolution(resolution, &x, &y)) {
                    ...
                    if ((i > 0) && (((item->getResource(i)->getHandlerType() == CH_LIBEXIF) && (item->getResource(i)->getParameter(_(RESOURCE_CONTENT_TYPE))
                                                                                                   == EXIF_THUMBNAIL))
...
Ref<CdsResourceManager::UrlBase> CdsResourceManager::addResources_getUrlBase(Ref<CdsItem> item, bool forceLocal)
{
    ...
    if (IS_CDS_ITEM(objectType) && (item->getFlag(OBJECT_FLAG_DVD_IMAGE))) {
        urlBase->urlBase = Server::getInstance()->getVirtualURL() + _(_URL_PARAM_SEPARATOR) + CONTENT_DVD_IMAGE_HANDLER + _(_URL_PARAM_SEPARATOR) + dict->encodeSimple() + _(_URL_PARAM_SEPARATOR) + _(URL_RESOURCE_ID) + _(_URL_PARAM_SEPARATOR);
        urlBase->addResID = true;
        return urlBase;
    }
#endif
