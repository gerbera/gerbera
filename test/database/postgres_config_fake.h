...
#ifndef GERBERA_PGSQL_CONFIG_FAKE_H
#define GERBERA_PGSQL_CONFIG_FAKE_H

class PostgresConfigFake : public Config {
public:
    fs::path getConfigFilename() const override { return {}; }
    std::string getOption(ConfigVal option) const override
    {
        if (option == ConfigVal::SERVER_STORAGE_PGSQL_HOST) {
            return "wombat.home";
        }
        if (option == ConfigVal::SERVER_STORAGE_PGSQL_USERNAME) {
            return "root";
        }
