...
TEST_F(ServerTest, ServerOutputsHelpInformation)
{
    ...
    auto expectedOutput = splitString(expected, '\n', '\0', true);
    for (auto&& line : expectedOutput) {
        ASSERT_THAT(output, HasSubstr(line));
    }
    for (auto&& pattern : patternList) {
        ASSERT_THAT(output, ContainsRegex(pattern));
    }

    patternList.push_back("/home/runner/\\.conan2.*"); // libuuid causes error message under conan :-(
    for (auto&& line : splitString(output, '\n', '\0', true)) {
        bool found = false;
        for (auto&& pattern : patternList) {
            std::smatch base_match;
            if (!line.empty() && std::regex_search(line, base_match, std::regex(pattern)))
                found = true;
        }
        if (!found) {
