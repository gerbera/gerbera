...
///@brief Provides generic helper methods to convert Duktape Context
/// data into c++ types
class DukTestHelper {
public:
    ...
    /// @brief Convert an array at the top of the stack
    /// to a c++ vector of strings
    static std::vector<std::string> arrayToVector(duk_context* ctx, duk_idx_t idx);

    /// @brief Convert an array of objects containing title property at the top of the stack
    /// to a c++ vector of strings
    static std::vector<std::string> containerToPath(duk_context* ctx, duk_idx_t idx);

    /// @brief Convert an object at the top of the stack
    ...
    /// @brief Adds a new object to the duk_context with meta data
    ...
    /// @brief Print error message based on duktape
    static void printError(duk_context* ctx, const std::string& message, const std::string& item);

private:
    /// @brief Extracts the property value given a complex associative array key
    ...
    /// @brief Returns true if the key is representing javascript array notation
    /// true = meta['dc:title']
    /// false = 'dc:title'
    /// false = 'meta'
