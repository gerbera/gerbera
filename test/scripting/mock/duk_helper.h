...
public:
    ...
    void createObject(duk_context* ctx, const std::map<std::string, std::string>& objectProps, const std::map<std::string, std::string>& metaProps);
...
private:
    ...
    std::tuple<std::string, std::string> extractObjectValues(duk_context* ctx, const std::string& key, duk_idx_t idx);
    ...
    bool isArrayNotation(const std::string& key);
};
