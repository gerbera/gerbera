...
if [ ! -f /var/run/gerbera/.config/gerbera/config.xml ]; then
    # Generate a config file with home set
  mkdir -p /var/run/gerbera/.config/gerbera
  gerbera --create-config --home /var/run/gerbera > /var/run/gerbera/.config/gerbera/config.xml
  ...
  sed 's/<import hidden-files="no">/<import hidden-files="no">\n\
    ...
    <\/autoscan>/' -i /var/run/gerbera/.config/gerbera/config.xml
fi
